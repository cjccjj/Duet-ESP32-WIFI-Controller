...
void Adafruit_SH1106_ESP32::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
Adafruit_SH1106_ESP32::Adafruit_SH1106_ESP32(int8_t SID, int8_t SCLK, int8_t DC, int8_t RST, int8_t CS) : Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  cs = CS;
  rst = RST;
  dc = DC;
  sclk = SCLK;
...
Adafruit_SH1106_ESP32::Adafruit_SH1106_ESP32(int8_t DC, int8_t RST, int8_t CS) : Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  dc = DC;
  rst = RST;
  cs = CS;
  hwSPI = true;
...
Adafruit_SH1106_ESP32::Adafruit_SH1106_ESP32(uint8_t reset) :
Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  sclk = dc = cs = sid = -1;
  rst = reset;
}

Adafruit_SH1106_ESP32::Adafruit_SH1106_ESP32(int8_t SDA, int8_t SCL) :
Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT) {
  sclk = dc = cs = sid = -1;
  sda = SDA;
  scl = SCL;
...
void Adafruit_SH1106_ESP32::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  _vccstate = vccstate;
  _i2caddr = i2caddr;
...
void Adafruit_SH1106_ESP32::invertDisplay(uint8_t i) {
  if (i) {
...
void Adafruit_SH1106_ESP32::sh1106_command(uint8_t c) { 
  if (sid != -1)
  {
    // SPI
...
void Adafruit_SH1106_ESP32::startscrollright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void Adafruit_SH1106_ESP32::startscrollleft(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void Adafruit_SH1106_ESP32::startscrolldiagright(uint8_t start, uint8_t stop){
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// display.scrollright(0x00, 0x0F) 
void Adafruit_SH1106_ESP32::startscrolldiagleft(uint8_t start, uint8_t stop){
  sh1106_command(SH1106_SET_VERTICAL_SCROLL_AREA);  
  sh1106_command(0X00);
  sh1106_command(SH1106_LCDHEIGHT);
...
void Adafruit_SH1106_ESP32::stopscroll(void){
...
void Adafruit_SH1106_ESP32::dim(boolean dim) {
  uint8_t contrast;
...
void Adafruit_SH1106_ESP32::sh1106_data(uint8_t c) {
  if (sid != -1)
  {
    // SPI
    //digitalWrite(cs, HIGH);
...
void Adafruit_SH1106_ESP32::display(void) {
  if (sid != -1)
  {
...
// clear everything
void Adafruit_SH1106_ESP32::clearDisplay(void) {
  memset(buffer, 0, (SH1106_LCDWIDTH*SH1106_LCDHEIGHT/8));
}


inline void Adafruit_SH1106_ESP32::fastSPIwrite(uint8_t d) {
...
void Adafruit_SH1106_ESP32::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
  boolean bSwap = false;
  switch(rotation) { 
    case 0:
...
void Adafruit_SH1106_ESP32::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void Adafruit_SH1106_ESP32::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
  bool bSwap = false;
  switch(rotation) { 
    case 0:
      break;
...
void Adafruit_SH1106_ESP32::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
